...
/**
 * @file
 * Documented API for interacting with Block Reference module.
 */
...
function hook_blockreference_blocks_pre_alter(&$blocks, $context) {
  // These are all the blocks that match the field instance's referenceable
  // module option. If you were to change a block's label now, that's the
  // label the autocomplete will try to match.
...
function hook_blockreference_blocks_post_alter(&$blocks, $context) {
  // These are the exact blocks in the result. If the widget is an
  // autocomplete, it will be cut off after 10 (?) elements, so sorting is
  // important. If you change the label here, the displayed labels will be
  // different than the matched labels, so probably don't do that.  Reverse
  // sort, because I'm like that.
